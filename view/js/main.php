    ...
    setupSearch() {
        ...
        const filterConcepts = () => {
            ...
            conceptItems.forEach(item => {
                ...
                const isRoot = !window.location.pathname.includes('/view/');
                const absoluteHref = (isRoot ? 'view/' : '') + hrefFile.replace('../', '');
...
const Router = {
    ...
    isInternalLink(link) {
        return link &&
               ...
               !link.hash &&
               link.target !== '_blank' &&
               (link.href.endsWith('.php') || link.href.endsWith('.php') || link.href.endsWith('/'));
    },
